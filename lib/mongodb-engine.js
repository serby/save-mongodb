...
module.exports = function (collection, engineOptions) {
  ...
  function castIdProperty(query) {
    ...
    try {
      if (typeof query[options.idProperty] === 'string') {
        newQuery[options.idProperty] = idCaster(query[options.idProperty])
      } else {
        _.each(newQuery[options.idProperty], function (value, key) {
          newQuery[options.idProperty][key] = value.map(idCaster)
        })
      }
    } catch (e) {
